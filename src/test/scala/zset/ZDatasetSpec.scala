...
import com.example.zset.{WeightType, ZSet, ZDataset}
import WeightType.{IntegerWeight, LongWeight, given}
import com.example.zset.ZDatasetOps.*

class ZDatasetSpec extends FunSuite {
  ...
  // === 基础构造测试 ===
  test("创建空的ZDataset") {
    val db = ZDataset.empty[String, Int]
    assertEquals(db.underlying.entryCount, 0)
    assert(db.underlying.isEmpty)
  }

  test("从可迭代对象创建ZDataset") {
    val db = ZDataset.fromIntIterable(List("a", "b", "a", "c"))
  ...
  test("从权重对创建ZDataset") {
    val db = ZDataset.fromPairs(List(("x", 2), ("y", 3), ("x", 1)))
  ...
  test("便捷构造方法 - weighted") {
    val db = ZDataset.weighted(("a", 2), ("b", 3))
    assertEquals(db.underlying.getWeight("a"), 2)
  ...
  // === SELECT 操作测试 ===
  test("select - 基本投影") {
    val persons = ZDataset.fromIntIterable(
      List(
        Person("Alice", 25),
        Person("Bob", 30),
        Person("Charlie", 25)
  ...
  test("select - 字符串长度投影") {
    val words   = ZDataset.fromIntIterable(List("hello", "world", "scala"))
  ...
  // === WHERE 操作测试 ===
  test("where - 基本过滤") {
    val numbers = ZDataset.fromIntIterable(List(1, 2, 3, 4, 5, 6))
  ...
  test("where - 字符串模式过滤") {
    val words       = ZDataset.fromIntIterable(List("apple", "banana", "apricot", "orange"))
  ...
  // === UNION 操作测试 ===
  test("union - 基本并集") {
    val db1 = ZDataset.fromIntIterable(List("a", "b", "c"))
    val db2 = ZDataset.fromIntIterable(List("c", "d", "e"))
  ...
  test("union - 带权重并集") {
    val db1 = ZDataset.fromPairs(List(("a", 2), ("b", 3)))
    val db2 = ZDataset.fromPairs(List(("a", 1), ("c", 4)))
  ...
  test("union - 空集合并集") {
    val db1 = ZDataset.fromIntIterable(List("a", "b"))
    val db2 = ZDataset.empty[String, Int]
  ...
  test("intersect - 基本交集") {
    val db1 = ZDataset.fromIntIterable(List("a", "b", "c"))
    val db2 = ZDataset.fromIntIterable(List("b", "c", "d"))
  ...
  test("intersect - 带权重交集") {
    val db1 = ZDataset.fromPairs(List(("a", 2), ("b", 3), ("c", 1)))
    val db2 = ZDataset.fromPairs(List(("b", 5), ("c", 2), ("d", 1)))
  ...
  // === EXCEPT 操作测试 ===
  test("except - 基本差集") {
    val db1 = ZDataset.fromIntIterable(List("a", "b", "c", "d"))
    val db2 = ZDataset.fromIntIterable(List("b", "c"))
  ...
  test("except - 带权重差集") {
    val db1 = ZDataset.fromPairs(List(("a", 2), ("b", 3), ("c", 1)))
    val db2 = ZDataset.fromPairs(List(("b", 5), ("e", 1)))
  ...
  // === AGG 操作测试 ===
  test("agg - 数字求和") {
    val numbers = ZDataset.fromIntIterable(List(1, 2, 3, 4, 5))
    val sum = numbers.agg(0, (acc, n) => acc + n)
    assertEquals(sum, 15)
  }

  test("agg - 字符串连接") {
    val words = ZDataset.fromIntIterable(List("Hello", " ", "World"))
    val concatenated = words.agg("", (acc, word) => acc + word)
    assertEquals(concatenated, "Hello World")
  }

  test("agg - 复杂对象聚合") {
    val persons = ZDataset.fromIntIterable(List(Person("Alice", 25), Person("Bob", 30), Person("Charlie", 35)))
  ...
  // === COUNT 操作测试 ===
  test("count - 基本计数") {
    val words = ZDataset.fromIntIterable(List("apple", "banana", "cherry"))
  ...
  test("count - 重复元素计数") {
    val numbers = ZDataset.fromPairs(List((1, 3), (2, 2), (1, 1))) // 注意权重会累加
  ...
  test("sum - 带权重求和") {
    val numbers = ZDataset.fromPairs(List((10, 2), (20, 3))) // 10*2 + 20*3 = 80
    val total = numbers.sum(identity)
    assertEquals(total, 80)
  }

  test("sum - 复杂对象属性求和") {
    val orders = ZDataset.fromIntIterable(List(
      Order(1, 100, 50.0),
      Order(2, 101, 75.0),
      Order(3, 102, 25.0)
    ))
  ...
  // === MAX 操作测试 ===
  test("max - 整数最大值") {
    val numbers = ZDataset.fromIntIterable(List(1, 5, 3, 9, 2))
    val maximum = numbers.max(identity)
    assertEquals(maximum, Some(9))
  }

  test("max - 复杂对象属性最大值") {
    val persons = ZDataset.fromIntIterable(List(
      Person("Alice", 25),
      Person("Bob", 30),
      Person("Charlie", 22)
    ))
  ...
  test("max - 空集合最大值") {
    val empty = ZDataset.empty[Int, Int]
  ...
  test("avg - 带权重平均值") {
    val numbers = ZDataset.fromPairs(List((10, 2), (20, 3))) // (10*2 + 20*3)/(2+3) = 80/5 = 16
    val average = numbers.avg(identity)
  ...
  // === DISTINCT 操作测试 ===
  test("distinct - 基本去重") {
    val numbers = ZDataset.fromPairs(List((1, 3), (2, 2), (1, 1))) // 权重会累加，然后distinct
  ...
  test("distinct - 复杂对象去重") {
    val persons = ZDataset.fromPairs(List(
      (Person("Alice", 25), 2),
      (Person("Bob", 30), 1),
      (Person("Alice", 25), 1)
    ))
  ...
  test("cartesianProduct - 基本笛卡尔积") {
    val colors = ZDataset.fromIntIterable(List("red", "blue"))
    val sizes = ZDataset.fromIntIterable(List("S", "M"))
  ...
  test("cartesianProduct - 数字组合") {
    val nums1 = ZDataset.fromIntIterable(List(1, 2))
    val nums2 = ZDataset.fromIntIterable(List(10, 20))
  ...
  // === SORTBY 操作测试 ===
  test("sortBy - 基本排序") {
    val numbers = ZDataset.fromIntIterable(List(3, 1, 4, 5)) // 移除重复元素，因为ZDataset会去重
  ...
  test("sortBy - 字符串长度排序") {
    val words = ZDataset.fromIntIterable(List("banana", "cat", "elephant"))
  ...
  test("take - 取超过总数的项") {
    val numbers = ZDataset.fromIntIterable(List(1, 2, 3))
  ...
  test("复合操作 - select + where + union") {
    val persons1 = ZDataset.fromIntIterable(List(
      Person("Alice", 25),
      Person("Bob", 30),
      Person("Charlie", 22)
    ))

    val persons2 = ZDataset.fromIntIterable(List(
      Person("David", 28),
      Person("Eve", 35)
    ))
